...
public:
    MapManager(std::weak_ptr<InputController> input_controller, std::function<void(Army*, const Hex&)> change_mode_function);
    ~MapManager();
    void printMap() const;
...
private:
    ...
    std::function<void(Army*, const Hex&)> change_mode_function_;
